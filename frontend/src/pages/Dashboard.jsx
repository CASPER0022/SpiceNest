...
export default function Dashboard() {
  ...
  // Generate continuous SVG Graph coordinates based on daily group data
  const generateGraphData = () => {
    // Group orders by date (sorted chronological)
    const grouped = {};
    
    // Fill in dates based on timeFilter to ensure smooth axis representation
    const now = new Date();
    let numDays = 7;
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white/80 p-4 rounded-2xl border border-gray-100 flex flex-wrap items-center justify-between gap-4 shadow-sm">
            <div className="flex flex-wrap gap-2">
              {['Last 7 days', 'This month', 'Last month', 'Year'].map(filter => (
                <button
                  key={filter}
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <Fragment key={order.id}>
                        ...
                        <td className="px-6 py-5 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="text-sm font-bold text-gray-800">{order.user?.name || 'Valued User'}</span>
                      ...
                      {/* Expanded Drawer panel for nested contents */}
                      {isExpanded && (
                        <tr>
                          <td colSpan="7" className="bg-gray-50/30 px-8 py-6 border-t border-b border-gray-100">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              
                              {/* Order Items Drawer Column */}
                              <div>
                                <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Line Items Summary</h5>
                                <div className="space-y-3 bg-white p-4 rounded-2xl border border-gray-150 shadow-sm max-h-[250px] overflow-y-auto">
                                  {order.items.map((item) => (
                                    <div key={item.id} className="flex justify-between items-center gap-4 text-xs font-semibold text-gray-700 pb-3 border-b border-gray-50 last:border-0 last:pb-0">
                                      <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center text-[10px] font-black shrink-0">
                                          {item.weight}
                                        </div>
                                        <div className="flex flex-col">
                                      ...
                                      <span className="font-extrabold text-gray-800">₹{(item.price * item.quantity).toFixed(2)}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>

                              {/* Shipping address Details Drawer Column */}
                              <div className="flex flex-col justify-between">
                                <div>
                                  <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Shipping Destination</h5>
                                  <div className="bg-white p-4 rounded-2xl border border-gray-150 shadow-sm space-y-3 text-xs">
                                    <div className="flex items-start gap-2">
                                      <MapPin size={16} className="text-gray-400 shrink-0 mt-0.5" />
                                      <div>
                                        <p className="font-bold text-gray-800">Recipient: {address.fullName || 'Valued Buyer'}</p>
                                        <p className="text-gray-500 font-medium mt-0.5">Phone: {address.mobileNumber}</p>
                                        <p className="text-gray-600 font-semibold mt-2 leading-relaxed">
                                ...
                                <div className="text-[10px] font-black text-emerald-800 bg-emerald-50 border border-emerald-100/50 p-3 rounded-xl uppercase tracking-widest mt-4">
                                  System Verified Stripe Invoice: {order.stripeSessionId.slice(0, 20)}...
                                </div>
                              </div>
